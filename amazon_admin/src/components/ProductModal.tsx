...
const ProductModal = ({ prod }: any) => {
  ...
  values.color.forEach((c: string) => formData.append("color[]", c));
  values.tags.forEach((t: string) => formData.append("tags[]", t));

  // ✅ Separate old images + new images
  const existingImages = values.images.filter((img: any) => img.url);
  const newImages = values.images.filter((img: any) => !img.url);

  // ✅ Send existing images as one JSON array
