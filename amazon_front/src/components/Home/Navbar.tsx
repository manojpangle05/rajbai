  ...
  return (
    <>
      <div className={`sm:static`}>
        <header className="header-top-strip py-2 hidden sm:block">
          <div className="w-full">
            <div className="flex justify-around">
              <div>
                <p className="text-white mb-0">Free shipping Over ₹2000</p>
              </div>
              <div>
                <p className="text-end text-white mb-0">Admin Email: <a className="text-[#fff]" href="support@rajbai.com">support@rajbai.com</a></p>
              </div>
            </div>
          </div>
        </header>
        ...
        <header className={`${active === true ? "shadow-sm fixed top-0 left-0 w-full z-10 transition-all ease-in-out " : null} w-full header-upper py-3`}>
          <div className="h-[100%] flex justify-between items-center">
            {screen ? <LiaTimesSolid onClick={screenFuncTrue} className="text-dark ml-4 text-[3rem] first-letter:block sm:hidden" /> :
              <AiOutlineMenu onClick={screenFuncFalse} className="text-dark ml-4 text-[3rem] first-letter:block sm:hidden" />
            }

            <div className="logo sm:w-1/6 w-5/12 text-center">
            
                <Link to="/" className="text-dark"><img src={Logo} className="bg-transparent" width="100" alt="logo" /></Link>
              
            </div>
            <div className="search sm:w-5/12 max-w-full">
              <form onSubmit={handleSubmit}>
                <div className="relative hidden sm:flex">
                  <button id="dropdown-button" data-dropdown-toggle="dropdown" className="flex-shrink-0 hidden z-10 md:flex items-center py-2.5 px-4 text-sm font-medium text-center  rounded-l-lg text-gray-900 bg-gray-100 hover:bg-gray-200  focus:outline-none focus:ring-gray-100 dark:bg-gray-700 dark:hover:bg-gray-600 dark:focus:ring-gray-700 dark:text-dark dark:border-gray-600"
                    type="button" onClick={category}> {selctedCat === "" ? "All categories " : selctedCat}<IoMdArrowDropdown size={22} /> </button>
                  {openCate &&
                    (
                      <div className="absolute left-0 top-10 z-10 mt-2 w-565 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black  focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="menu-button" >
                  ...
                  <div className="relative w-full">
                    ...
                    <button type="submit" className="group bg-[#FBBE01] absolute top-0 right-0 p-2.5 text-sm font-medium h-full text-dark rounded-r-lg">
                      <BsSearch size={22} />
            ...
            <div className=' w-full links sm:w-5/12 max-w-4xl'>
              <div className="header-upper-links w-full flex items-center justify-around ">
                {/* <div className="group bg-transparent">
                  <Link to="/compare" className="flex items-center text-dark">
                ...
                <div className="group bg-transparent">
                  <Link to="/wishlist" className="flex items-center  text-dark">
                    <img src={Wishlist} alt="wishlist" className="ml-[5px] transition-all transform group-hover:-scale-x-100 duration-500 ease-in-out" />
                    <p className="mb-0 text-[1rem] hidden lg:block">Wishlist</p>
                  </Link>
                </div>
                <div className="group bg-transparent" >
                  {user ? (
                    <>
                      <div onClick={handleLogout} className="flex sm:cursor-pointer items-center text-dark">
                        <AiOutlineLogout title="logout" size={35} className="transition-all transform group-hover:-scale-x-100 duration-500 ease-in-out" />
                        <p className="mb-0 text-[1rem] hidden lg:block">logout</p>
                      </div>
                    </>
                  ) : (
                    <>
                      <Link to="/login" data-tooltip-id="Login_tooltip" className="flex items-center text-dark">
                        <img src={User} alt="user" className="ml-[5px] transition-all transform group-hover:-scale-x-100 duration-500 ease-in-out" />
                        <p className="mb-0 text-[1rem] hidden lg:block"> My Account</p>
                      </Link>
                    </>
                  )
                  }
                ...
                <div className="group bg-transparent">
                  <Link to="/cart" className="flex items-center">
                    ...
                    <div className="flex flex-col">
                      ...
                      <p className="mb-0 text-dark text-[1rem]">
          ...
          <div className="flex justify-between  items-center">
            <div className="relative inline-block text-left">
              <div className="flex items-center ">
                ...
                <button onClick={options} className="text-gray-900 bg-gray-100 hover:bg-gray-200  focus:outline-none  dark:bg-gray-700 dark:hover:bg-gray-600 dark:focus:ring-gray-700 dark:text-dark dark:border-black sm:inline-flex w-full hidden justify-center gap-x-1.5 rounded-md  px-3 py-2 text-sm font-semibold shadow-md" id="menu-button" aria-expanded="true" aria-haspopup="true">
                  <IoMdKeypad size={22} />
                  Categories
              ...
              </div>
              {
                dropdown && (
                  ...
                  <div className="bg-white dark:bg-transparent absolute sm:block hidden right-0 z-10 mt-2 w-565 absolute left-0 top-10 z-10 mt-2 w-565 origin-top-left rounded-lg shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="menu-button" >
                    
                      <div className=" rounded-md bg-white shadow-lg ring-1 ring-black  focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="menu-button" >
                        <div className="py-1" role="none">
                          {categories && categories.map((cat: Category) => (
                            <div onClick={() => { handleCategory(cat?._id); setSelectedCat(cat?.title) }} key={cat?._id} className="hover:text-black hover:bg-gray-200 bg-opacity-90 pl-5 my-2">{cat?.title}</div>
                          ))}
            ...
            {/* <PiDotsThreeVerticalBold className="text-dark hidden sm:block" size={30} /> */}
            <div className="flex items-center text-dark uppercase">
              <NavLink className="mr-[20px] border-none" to="/"> <span>Home</span></NavLink>
              <NavLink className="mr-[20px] border-none" to="/products"><span>our store</span></NavLink>
              <NavLink className="mr-[20px] border-none" to="/blogs"><span>blogs</span></NavLink>
              <NavLink className="mr-[20px] border-none" to="/contact"><span>contact</span></NavLink>
            </div>
          </div>
          {
            isDropdownOpen && (
